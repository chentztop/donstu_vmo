...
def write_bmp_image(filename, data):
    with open(filename, 'wb') as file:
        file.write(data)  # Теперь data должен быть байтами



def compress_bmp_image(input_filename, output_filename, algorithm):
    data = read_bmp_image(input_filename)
    if algorithm == "shannon_fano":
        codebook, compressed_data = compress_shannon_fano(data)
        # Преобразуем compressed_data в байты перед записью
        compressed_bytes = compressed_data.encode('utf-8')
        write_bmp_image(output_filename, compressed_bytes)
    elif algorithm == "lzss":
        compressed_data = compress_lzss(data)
        # Преобразуем compressed_data в байты перед записью
        compressed_bytes = compressed_data.encode('utf-8')
...
def decompress_bmp_image(input_filename, output_filename, algorithm):
    data = read_bmp_image(input_filename)
    if algorithm == "shannon_fano":
        codebook_str, compressed_data = data.split(b'\n', 1)
        codebook = eval(codebook_str.decode())
        decompressed_data = decompress_shannon_fano(codebook, compressed_data)
        write_bmp_image(output_filename, decompressed_data)
    elif algorithm == "lzss":
        compressed_data = eval(data)
        decompressed_data = decompress_lzss(compressed_data)
        write_bmp_image(output_filename, decompressed_data)

# Создаем новое изображение размером 100x100 пикселей
image = Image.new('RGB', (100, 100), color = (73, 109, 137))
image.save('source.bmp')
compress_bmp_image('source.bmp', 'compressed.bmp', 'shannon_fano')

decompress_bmp_image('compressed.bmp', 'output.bmp', 'shannon_fano')
